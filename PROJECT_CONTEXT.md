...
- **This Week's Lineup Risks side panel** (replaces earlier injury news attempt): per-team lineup risks with severity scoring, week filter dropdown, play-probability pills — Q/D/O are **measured** from real nflverse report data, not guessed (~57% Q, ~1% D, 0% O; see **Practice Report findings**' follow-up measurement for the real sample sizes), while IR/PUP/NA/SUSP stay at 0% because that reflects an actual NFL rule, not something more data could refine — status-specific hover tooltips state which is which
...
- **The official `report_status` is a much sharper "will they play" signal
  ...
  designation → 84.8% (n=7,044, the population baseline) — this specific
  cut is QB/RB/WR/TE only, 2018-2025, matching this notebook's SEASONS
  scope. **Follow-up (Sep 2026): re-measured with K included and the FULL
  2009-2025 history (27,778 real report rows) specifically to recalibrate
  the Lineup Risks panel's `playProbability()` (index.html), which had
  been a hand-picked heuristic (~75% Questionable, ~25% Doubtful, 0% Out)
  from before this data existed** — Out 0.058% (n=5,161), Doubtful 1.14%
  (n=1,057), Questionable 56.7% (n=7,180). **Done, not just flagged**:
  `playProbability()` now returns these measured rates (rounded: ~57%/
  ~1%/0%), and every tooltip states the real sample size behind it so a
  future editor doesn't mistake these for guesses again. IR/PUP/NA/SUSP
  were checked too and deliberately left unchanged — nflverse's weekly
  report has no equivalent concept at all (`report_status` is only ever
  Questionable/Doubtful/Out/Probable), so there's no real rate to measure
  for a roster-level designation like IR; their 0% reflects an actual NFL
  rule (IR's 4-game minimum, PUP's practice/play restriction, a
  suspension's formal ineligibility), not an estimate open to refinement.
